...
This section is **optional** (`0..1`). Imaging study metadata may not be available to all systems or for all types of reports (e.g. reports from dentistry or dermatology, or legacy data migrated from systems that do not retain study details). A report may therefore be created without a fully populated Imaging Study section. When the section is present but no entry can be provided, the reason SHALL be conveyed using `section.emptyReason`, and any study information that is known (e.g. modality, anatomy) may instead be described in the narrative and in the [Procedure](#procedure) section.

When the report creator has access to an existing `ImagingStudy` resource, or knows the Study Instance UID, this information SHALL be populated. Where no `ImagingStudy` resource instance is available, the study MAY still be referenced by business identifier by populating the Study Instance UID on the reference `identifier` without a `reference` (see the [FHIR reference semantics](https://hl7.org/fhir/references.html#Reference)).

The Study Instance UID is the primary identifier of the study and is the key needed to retrieve the [Manifest-based Access to DICOM objects](https://profiles.ihe.net/RAD/MADO/) and, through it, to locate and access the images associated with the report. For this reason the [[[StudyInstanceUidIdentifierEuImaging]]] carries a `SHALL:populate-if-known` obligation: whenever the report creator knows the Study Instance UID it must be persisted in the report.
