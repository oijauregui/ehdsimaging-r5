...
* **Retraction** — the report was issued in error and is withdrawn, for example because it was assigned to the wrong patient or study. The original report is then replaced by an empty `Composition` and empty `DiagnosticReport` (no clinical content, and a minimal explanatory narrative) with the value of `Composition.status` and `DiagnosticReport.status` set to `entered-in-error`.
